...
if(arrange.equals("true")) {
	...
	if(day != null ) {
		...
		if(!jar.isEmpty()) {
			for(String i : day) {
				...
				sql = "DELETE FROM `staff-arrange`  WHERE `day` = '"+i+"' and `orderWork` ='"+obj.get("workTime")+"' and `year` = '"+year+"' and `month` = '"+(Integer.parseInt(month)+2)+"' and `user` = '"+user+"'";
	...
	if(day != null ) {
		for(String i : day.substring(1,day.length()-1).split(",")) {
	...
	if(nameArray != null ) {
		for(String i : nameArray) {
			if(!i.equals("無")) {
				...
				Integer monthInt = (Integer.parseInt(month)+2);
				sql = "DELETE FROM `staff-worktime`  WHERE `day` = '"+day+"' and `orderWork` ='"+orderWork+"' and `year` = '"+year+"' and `month` = '"+monthInt+"'";
				a.executeSQLUpdate(sql);
				sql = "INSERT INTO `staff-worktime`( `user`,`name`, `year`, `month`, `day`, `orderWork`) VALUES ('"+user+"','"+name+"','"+year+"','"+monthInt+"','"+day+"','"+orderWork+"')";
				a.executeSQLUpdate(sql);
				if(!orderWork.equals("morning")){
					user = i.substring(i.lastIndexOf("(")+1, i.lastIndexOf(")"));
					name = i.substring(i.indexOf(")")+1, i.lastIndexOf("("));
					sql = "INSERT INTO `staff-worktime`( `user`,`name`, `year`, `month`, `day`, `orderWork`) VALUES ('"+user+"','"+name+"','"+year+"','"+monthInt+"','"+day+"','"+orderWork+"')";
					a.executeSQLUpdate(sql);
				}
			}
